...
class Commands(list):
    ...
    """
    def stack_cycle(self, env, callback, load_counter=True, return_counter_var=False):
    ...
    После завершения на стек помещается число совершенных итераций.
    """
    def memory_cycle(self, env, var_number, callback=None, load_counter=True, return_counter_var=False):
        # Создаем метки и переменные, необходимые для прохождения цикла.
        start_while_label = Environment.create_label(env)
        end_while_label = Environment.create_label(env)
        continue_label = Environment.create_label(env)
    ...
    После завершения на стек помещается число совершенных итераций.
    """
    def memory_heap_cycle(self, env, var_number, callback=None, load_counter=True, return_counter_var=False):
        # Создаем метки и переменные, необходимые для прохождения цикла.
        start_while_label = Environment.create_label(env)
        end_while_label = Environment.create_label(env)
        continue_label = Environment.create_label(env)
